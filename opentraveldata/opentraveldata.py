...
class OPTDDownloadedFileSizeError (OPTDDownloadFileError):
   """
...
class FileType(enum.Enum):
   """
   Type of the OPTD file. For now, either main (IATA/ICAO) or UNLC (UN/LOCODE).
   """
   MAIN = enum.auto()
   UNLC = enum.auto()
...
class OpenTravelData():
   ...
   def assumeFilesExist(self):
      """
      Check whether the data files have been downloaded properly.
      If not, raise an exception with hints on remediation.
      """
      do_files_exist = self.doLocalFilesExist()
      if not do_files_exist:
         err_msg = f"[OpenTravelData::assumeFilesExist] At least one of the data files ({self.local_iata_por_filepath} or {self.local_unlc_por_filepath}) has not been downloaded properly. " \
            f"You can execute the downloadFilesIfNeeded() again to remediate the issue. " \
            f"Check also that you have access rights for writing in the local directory ({self.local_dir}) where the data files are to be downloaded and stored. " \
            f"If you do not have writing access rights to {self.local_dir}, re-initialize the OpenTravelData object by passing the path of a directory, as local_dir, on which you have writing access rights: " \
            f"myOPTD = opentraveldata.Opentraveldata(local_dir='/directory-where-you-have-writing-access-rights')"
   ...
   def fileSizes(self):
      """
      Return the sizes of the data files.
      """
   ...
   def humanFileSizes(self):
      """
      Return the sizes of the data files in MB.
      """
      # Retrieve the sizes of the data files in Bytes
      (optd_por_file_size, optd_unlc_file_size) = self.fileSizes()

      # Convert the sizes into MB
      optd_por_file_size_in_mb = optd_por_file_size / (1024 * 1024)
      optd_unlc_file_size_in_mb = optd_unlc_file_size / (1024 * 1024)

      # Display
      optd_por_file_size_str = f"{optd_por_file_size_in_mb:.2f} MB"
      optd_unlc_file_size_str = f"{optd_unlc_file_size_in_mb:.2f} MB"

      return (optd_por_file_size_str, optd_unlc_file_size_str)

   def validateFileSizes(self):
      """
      ...
      if 4e6 <= optd_unlc_file_size <= 5e6:
         ...
         err_msg = f"[OpenTravelData::validateFileSizes] File size of {self.local_unlc_por_filepath} not within range (expected to be 4-5 MB)"
         raise OPTDDownloadedFileSizeError(err_msg)
   ...
   def downloadFilesIfNeeded (self):
      ...
      # Validate the size of the downloaded data files
      self.validateFileSizes()
